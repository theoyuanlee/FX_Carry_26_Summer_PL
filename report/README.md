...
1. **Every number traces to a committed CSV.** No figure appears in prose without a file behind
   it — the plan document's opening promise, and the thing that caught four base defects. Almost all
   of them are in `cesare/outputs/`; the one documented exception is data provenance, where ch. 2
   cites `data/raw/ticker_manifest.csv`. An earlier wording of this rule said "in `cesare/outputs/`"
   and was narrower than the report it describes.
