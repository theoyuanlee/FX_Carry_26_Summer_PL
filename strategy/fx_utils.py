"""Shared helpers for the FX carry project — the engine.

Data loading, currency-return construction (spot and forward-implied carry),
performance statistics and factor regressions. This module is the low-level
library; `strategy.core.run` composes these functions into the team's baseline
backtest, and every teammate's extension is expected to go through that entry
point rather than re-wiring the chain by hand. Direct use is still fine for
research that needs a piece the config does not expose.

Lives in `strategy/` so it is team-owned; `cesare/fx_utils.py` remains as a
re-export shim so existing notebooks that `import fx_utils` keep working.
"""
