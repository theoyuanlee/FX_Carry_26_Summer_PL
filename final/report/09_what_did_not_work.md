...
**And it is not on the common window, so it is not directly comparable to anything else in this
report.** The battery runs 2007-05 to 2024-09 — **4,545 trading days against the shared base's
5,001** — on a 7-name universe whose own carry anchor is negative. Window, universe and anchor all
differ. That does not rescue the signal (a book that cannot beat a *negative* anchor is not saved by
a better one), but "tested on the shared base" would overstate what this file shows, and every other
null in this chapter *was* measured on the base.
...
**Mean out-of-sample AUC: 0.4685** — worse than a coin flip. That averages the **6 of 13**
purged folds in which AUC is defined at all; the other seven contain no tail month, as the
next paragraph explains. A null can be overstated exactly like a positive one, so the
